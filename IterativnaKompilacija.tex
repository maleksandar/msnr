...
\par
Sa druge strane postoji velika potražnja za softverom koji rešava raznovrsne probleme, a koji se pritom izvršava na još raznorodnijim arhitekturama i platformama.
Veliki broj problema koji može biti rešen softverski sa jedne strane i nedovoljan broj razvijalaca softvera sa druge nam govori da je brz razvoj sofvera neophodan kako bi svi problemi 
...
naprednije mogućnosti modernih arhitektura procesora. Korišćenjem samo osnovnih hardverskih funkcionalnosti procesora dolazimo do programa koji su veoma malo (ako i uopšte) optimizovani.
...
Prvi način testiranja je pokazao da su za fiksiranu veličinu ulaznih podataka preformanse dobijenih programa značajno poboljšane.
U praksi naš program se neće izvršavati stalno nad ulaznim podacima koji su iste veličine kao i oni koji su korišćeni 
pri kompilaciji, pa se iz tog razloga pristupilo i drugom načinu testiranja. Kod drugog načina testiranja 
...
srednja vrednost dobijenih rezultata. Na slici \ref{fig:slika3} prikazani su dobijeni rezultati.
Crnom bojom su prikazani rezultati dobijeni prvim načinom testiranja. Sivom bojom su prikazani rezultati dobijeni
drugim načinom testiranja s tim što su veličine ulaznih podataka sledeće:
\begin{itemize}
\item 256, 300 i 301 za MxM i FDCT
\item 2048, 2300 i 2301 za MxV i RECO
\end{itemize}
Belom bojom su prikazani rezultati dobijeni drugim načinom testiranja, a veličine ulaznih podataka su:
\begin{itemize}
...
Ako pogledamo grafik videćemo da se rezultati dobijeni drugom tehnikom idalje daju značajna poboljšanja u brzini izvršavanja.
Možemo primetiti da je brzina izvršavanja programa dobijenih drugom tehnikom su za nijansu sporija od programa dobijenih 
prvom tehnikom. U slučaju MxM vidimo da je program dobijen drugom metodom sa 3 različite veličine ulaznih podataka brži od programa
dobijenim prvim načinom testiranja.

\begin{figure}[p]
\begin{center}
\includegraphics[width=\textwidth]{performanse3}
\end{center}
